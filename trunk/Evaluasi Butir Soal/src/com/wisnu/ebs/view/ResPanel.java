...
import com.wisnu.ebs.controller.ResController;
 ...
 * @author Wisnu Citra
 */
public class ResPanel extends javax.swing.JPanel implements ResListener {

    private final String colHeader1[] = {"T.Kesukaran", "Daya Beda", "Korelasi"};
    String colHeader2[];
    private final String colHeader3[] = {"Jumlah Benar", "Jumlah Salah", "Skor Dibobot"};
    private final String colHeader4[] = {"Daya Beda (%)", "T. Kesukaran", "Korelasi", "Sign. Korelasi"};
    ...
    public void onLoad() {
        ...
        controller.correcting();
        controller.tk();
        controller.db();
        controller.ep();
        controller.reliability();
        controller.validity();
        controller.rightAndWrong();

        for (int i = 0; i < row; i++) {
            rowHeader[0][i] = String.valueOf("Soal " + (i + 1));
            for (int j = 0; j < 3; j++) {
                if (j == 0) {
                    dataTable[0][i][j] = String.valueOf((float) resModel.getTk()[i] / row);
                }
                if (j == 1) {
                    dataTable[0][i][j] = String.valueOf(new DecimalFormat("#.##").format(resModel.getDb()[i] * 100)) + "%";
                }
                if (j == 2) {
                    dataTable[0][i][j] = resModel.getCorrelation()[i];
                }
        ...
        for (int i = 0; i < row; i++) {
            ...
            for (int j = 0; j <= (tipe + 1) + 1; j++) {
                if (j == 0) {
                    dataTable[1][i][j] = String.valueOf(resModel.getIPc()[0][i]);
                }
                if (j == 1) {
                    dataTable[1][i][j] = String.valueOf(resModel.getIPc()[1][i]);
                }
                if (j == 2) {
                    dataTable[1][i][j] = String.valueOf(resModel.getIPc()[2][i]);
                }
                if (j == 3) {
                    dataTable[1][i][j] = String.valueOf(resModel.getIPc()[3][i]);
                }
                if (tipe == 5 && j == 4) {
                    dataTable[1][i][j] = String.valueOf(resModel.getIPc()[4][i]);
                }
                if ((tipe == 4 && j == 4) || (tipe == 5 && j == 5)) {
                    dataTable[1][i][j] = resModel.getEp()[i];
                }
                if ((tipe == 4 && j == 5) || (tipe == 5 && j == 6)) {
                    if (resModel.getEp()[i].equals("Tidak Efektif")) {
        ...
        for (int i = 0; i < col; i++) {
            rowHeader[2][i] = resModel.getSortedNumber()[i][0];
            for (int j = 0; j < 3; j++) {
                if (j == 0) {
                    dataTable[2][i][0] = String.valueOf(resModel.getNRaW()[i][0]);
                }
                if (j == 1) {
                    dataTable[2][i][1] = String.valueOf(resModel.getNRaW()[i][1]);
                }
                if (j == 2) {
                    dataTable[2][i][2] = String.valueOf(new DecimalFormat("#.##").format(((float) resModel.getNRaW()[i][0] / row) * 100));
                }
            }
        ...
        for (int i = 0; i < row; i++) {
            ...
            for (int j = 0; j < 4; j++) {
                if (j == 0) {
                    if (resModel.getDb()[i] <= 0.20) {
                        dataTable[3][i][0] = "Buruk";
                    } else if (resModel.getDb()[i] > 0.20 && resModel.getDb()[i] <= 0.40) {
                        dataTable[3][i][0] = "Cukup";
                    } else if (resModel.getDb()[i] > 0.40 && resModel.getDb()[i] <= 0.70) {
                ...
                if (j == 1) {
                    if ((float) resModel.getTk()[i] / row <= 0.30) {
                        dataTable[3][i][1] = "Sukar";
                    } else if ((float) resModel.getTk()[i] / row > 0.30 && (float) resModel.getTk()[i] / row <= 0.70) {
                ...
                if (j == 2) {
                    dataTable[3][i][j] = resModel.getCorrelation()[i];
                }
                if (j == 3) {
                    if (row <= 10) {
                        if (resModel.getCorrelationNumber()[i] >= 0.576 && resModel.getCorrelationNumber()[i] < 0.708) {
                            dataTable[3][i][j] = "Signifikan";
                        } else if (resModel.getCorrelationNumber()[i] >= 0.708) {
        ...
        labRata.setText(resModel.getTempData()[0]);
        labSimpangan.setText(resModel.getTempData()[1]);
        labKorelasi.setText(resModel.getTempData()[2]);
        labReabilitas.setText(resModel.getTempData()[3]);
    }

    public void setMainModel(MainModel model) {
        this.mainModel = model;
        this.resModel = new ResModel();
        this.resModel.setListener(this);
        this.controller = new ResController();
        this.controller.setResModel(resModel);
        this.controller.setMainModel(mainModel);
        this.controller.fireOnload();
    }

    public void initData() {
        this.aktif = mainModel.getBerkasAktif();
        this.col = Integer.parseInt(mainModel.getJmlSiswa()[aktif]);
        this.row = Integer.parseInt(mainModel.getJmlSoal()[aktif]);
        this.tipe = Integer.parseInt(mainModel.getTipeSoal()[aktif]);

        colHeader2 = new String[this.tipe + 2];

        dataTable[0] = new String[row][3];
        dataTable[1] = new String[row][this.tipe + 2];
        dataTable[2] = new String[col][3];
        dataTable[3] = new String[row][4];
